...
namespace Umizoo.Messaging
{
    ...
    {
        private static readonly string[] IgnoreExceptionProperties =
        {
            "ClassName",
            "Data",
            "InnerException",
        ...
        protected override PublishableExceptionDescriptor Convert(Envelope<IPublishableException> envelope,
            ITextSerializer serializer)
        {
            ...
            var items = new List<PublishableExceptionDescriptor.PropertyEntry>();
            envelope.Body.GetObjectData(serializableInfo, new StreamingContext(StreamingContextStates.Clone));

            for (var enumerator = serializableInfo.GetEnumerator(); enumerator.MoveNext();)
            {
                if (enumerator.Current.Name.InArray(IgnoreExceptionProperties))
                    continue;

                var serializationEntry =
                    new PublishableExceptionDescriptor.PropertyEntry(enumerator.Current.ObjectType)
                    {
