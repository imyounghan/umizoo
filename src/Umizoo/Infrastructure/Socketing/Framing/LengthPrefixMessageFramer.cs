...
namespace Umizoo.Infrastructure.Socketing.Framing
{
            ...
            for (int i = bytes.Offset, n = bytes.Offset + bytes.Count; i < n; i++) {
                if (_headerBytes < HeaderLength) {
                    ...
                    try {
                        ...
                        LogManager.Default.Error(ex, "Parse message buffer failed, _headerLength: {0}, _packageLength: {1}, _bufferIndex: {2}, copyCnt: {3}, _messageBuffer is null: {4}",
                            _headerBytes,
                            _packageLength,
                            _bufferIndex,
                            copyCnt,
                    ...
                    if (_bufferIndex == _packageLength) {
                        if (_receivedHandler != null) {
                            try {
                                ...
                                LogManager.Default.Error("Handle received message failed.", ex);
                            }
                        }
                        _messageBuffer = null;
                        _headerBytes = 0;
