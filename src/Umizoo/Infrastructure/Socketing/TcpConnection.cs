...
namespace Umizoo.Infrastructure.Socketing
{
    ...
    using Umizoo.Infrastructure.Socketing.Framing;
    ...
    public class TcpConnection : ITcpConnection
    {
        ...
        private void OnMessageArrived(ArraySegment<byte> messageSegment)
        {
            ...
            try {
                ...
                LogManager.Default.Error("Call message arrived handler failed.", ex);
            }
        }
        ...
        private void CloseInternal(SocketError socketError, string reason, Exception exception)
        {
            if (Interlocked.CompareExchange(ref _closing, 1, 0) == 0) {
                try {
                    ...
                    LogManager.Default.Error("Return receiving socket event buffer failed.", ex);
                }
                ...
                if (!isDisposedException) {
                    if (LogManager.Default.IsInfoEnabled) {
                        LogManager.Default.Info(exception, "Socket closed, remote endpoint:{0} socketError:{1}, reason:{2}.", RemotingEndPoint, socketError, reason);
                    }
                }
                _socket = null;
                ...
                if (_connectionClosedHandler != null) {
                    try {
                        ...
                        LogManager.Default.Error("Call connection closed handler failed.", ex);
                    }
                }
            }
        }
